...
- **Audit & system logging (Reports)** — the Reports area has permission-gated tabs: **Operational**, **Audit Trail** (business events), **System Log**, and **User Access & Groups**. The System Log is a dedicated append-only Postgres table capturing security/system events — sign-ins, failed attempts, lockouts, sign-outs, invalid sessions, permission denials, administrative changes, imports, and errors — each with source **IP** and a severity level (`SECURITY`/`WARN`/`ERROR`/`INFO`), filterable and CSV-exportable. The access report lists group permissions and per-user effective access with recertification dates and SoD flags. Gated by `logs.view` / `audit.view` / `admin.manage`.
